...
export const UnitDialog: React.FC<UnitDialogProps> = ({ isOpen, onClose, unit }) => {
  ...
  } = useBuildings(keycloak?.token, keycloak?.realm)

  const {
    data: building,
    isLoading: buildingIsLoading,
    isError: buildingIsError,
  } = useBuilding(selectedBuildingId, keycloak?.token, keycloak?.realm)
  ...
  const {
    ...
    isError: bedsIsError,
  } = useBeds(selectedFloorId, true, keycloak?.token, keycloak?.realm)
    ...
    try {
      if (isEditing) {
        if (unitName != unit.name) {
          await updateUnit(unit.unitId, { name: unitName }, keycloak?.token, keycloak?.realm)
        }
        ...
        if (!arraysAreEqual(bedIds, selectedBedIds)) {
          await updateUnitBedAssociations(
            unit.unitId,
            selectedBedIds,
            keycloak?.token,
            keycloak?.realm
        ...
        const { unitId: newUnitId } = await postUnit(unitName, keycloak?.token, keycloak?.realm)
        const unitBedAssociations = Object.entries(selectedBeds)
          .filter(([, val]) => val)
          .map(([key]) => ({ unitId: newUnitId, bedId: key }))

        await postUnitBedAssociations(unitBedAssociations, keycloak?.token, keycloak?.realm)
      }
