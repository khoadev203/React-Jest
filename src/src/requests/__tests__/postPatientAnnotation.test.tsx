...
describe('request patientAnnotation', () => {
  it('should get valid params', async () => {
    ...
    const token = ''
    const realm = 'ox'
    const errorMessage = 'either token, patientAnnotation or realm is missing.'
    const error = postPatientAnnotation(patientAnnotation, token, realm)
  ...
  it('request should succeed', async () => {
    ...
    const postResponse = await postPatientAnnotation(patientAnnotation, token, realm)
