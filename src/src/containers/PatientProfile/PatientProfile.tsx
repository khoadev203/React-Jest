...
const PatientProfile: React.FC = () => {
  ...
  } = usePatientAnnotations(patient?.patientId, keycloak?.token, keycloak?.realm)
  ...
  const backUrl = unit
    ? `/${router.query?.realmName}/units/${unit.unitId}`
    : `/${router.query?.realmName}/units`
  ...
  const handleRemovePatient = async (): Promise<void> => {
    ...
    try {
      await deletePatient(patientId, keycloak?.token, keycloak?.realm)
      mutateLocalUnits(
        [
  ...
  const handleSavePatient = async (): Promise<void> => {
    ...
    try {
      if (patientId === null) {
        const patientResponse = await postPatient(
          submitPatientForm,
          keycloak?.token,
          keycloak?.realm
        )
        ...
        if (patientResponse && clinicalNotesForm) {
          ...
          revalidatePatientAnnotations(patientId, keycloak?.token, keycloak?.realm)
        }

        router.push(backUrl)
      } else {
        await updatePatient(patientId, submitPatientForm, keycloak?.token, keycloak?.realm)
        revalidateUnits(keycloak?.token, keycloak?.realm)
