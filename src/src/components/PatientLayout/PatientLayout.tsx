...
import { eventToBedStatus, flattenQueryString, isWSEventMessage, isWSMessage } from '@/lib/utils'
...
export const PatientLayout: React.FC<PatientLayoutProps> = ({
  ...
  const { data: wsTicket, isError: wsTicketIsError } = useWsTicket(keycloak?.token, keycloak?.realm)
      ...
      try {
        ...
        if (isWSMessage(message)) {
          if (isWSEventMessage(message)) {
            setBedStatus(eventToBedStatus(message.event))
          }
        }
      } catch (error) {
  ...
  return (
    <Layout>
      ...
      <Header className="flex items-center">
        <Link href={`/${router.query?.realmName}/units/${unitId}`} passHref={!isLoading}>
          <IconLink label="Back">
            <ArrowIcon />
          </IconLink>
        </Link>
      ...
      <DetailLayout>
        <DetailNav>
          <DetailNavList>
            <OptionalLink href={`/${router.query?.realmName}${hrefBase}/profile`}>
            ...
            <OptionalLink
              href={isEmptyBed ? null : `/${router.query?.realmName}${hrefBase}/events`}
            >
